...
Usage:
    poetry run python scripts/treesitter_to_datalog.py
"""
...
_SCOPE_OPENERS: frozenset[str] = frozenset([
    ...
    "program", "source_file", "compilation_unit", "translation_unit",
    # types
    "class_declaration", "interface_declaration", "enum_declaration",
    "record_declaration", "annotation_type_declaration",
    # callables
    "method_declaration", "constructor_declaration", "function_declaration",
    "function_definition", "arrow_function", "lambda_expression",
    # blocks
    "class_body", "block", "constructor_body",
    # control flow with own scope
    "for_statement", "enhanced_for_statement",
    "while_statement", "do_statement",
    "try_statement", "catch_clause", "switch_block",
    # other languages
    "namespace_body", "module",
])
...
@dataclass(frozen=True)
class RefersToFact:
    ref_id: int; decl_id: int
    def __str__(self) -> str:
        return f"refers_to({self.ref_id}, {self.decl_id})."

@dataclass(frozen=True)
class CallFact:
    site_id: int; receiver_id: int | None; method_name: str
    def __str__(self) -> str:
...
class RichDatalogFactSet:
    ...
    def all_facts(self) -> Iterator:
        for bucket in [
            self.nodes, self.tokens, self.positions, self.parents,
            self.contains, self.fields, self.names, self.declared_types,
            self.scopes, self.scope_parents, self.declares,
            self.declarations, self.references, self.refers_to,
            self.calls, self.instantiations,
        ]:
            yield from bucket

    def to_souffle_facts(self, output_dir: Path) -> None:
        ...
        write("position",     [f"{f.node_id}\t{f.start_row}\t{f.start_col}\t{f.end_row}\t{f.end_col}"  for f in self.positions])
        ...
        write("field",        [f"{f.parent_id}\t{sym(f.field_name)}\t{f.child_id}"                      for f in self.fields])
        write("name",         [f"{f.node_id}\t{sym(f.text)}"                                            for f in self.names])
        write("declared_type",[f"{f.node_id}\t{sym(f.type_text)}"                                       for f in self.declared_types])
        write("scope",        [f"{f.scope_id}\t{sym(f.kind)}"                                           for f in self.scopes])
        write("scope_parent", [f"{f.inner_id}\t{f.outer_id}"                                            for f in self.scope_parents])
        ...
        write("call",         [f"{f.site_id}\t{-1 if f.receiver_id is None else f.receiver_id}\t{sym(f.method_name)}" for f in self.calls])
        write("instantiation",[f"{f.site_id}\t{sym(f.type_name)}"                                       for f in self.instantiations])
...
class _Emitter:
    ...
    def _walk(
        ...
        f.positions.append(PositionFact(
            our_id,
        ...
        if node.type in _NAME_NODE_TYPES and node.child_count == 0:
            ...
            parent_type = self._id_to_type.get(parent_id, "") if parent_id is not None else ""
            is_decl = (
                field_name == "name"
                and parent_type in self._declaration_parents
            )
        ...
        if node.type == "method_invocation":
            ...
            for idx, child in enumerate(node.children):
                fname = node.field_name_for_child(idx)
                cid = self._child_id(child)
                if fname == "object":
                    receiver_id = cid
                elif fname == "name":
                    method_name = self._text(child)
            if method_name is not None:
                f.calls.append(CallFact(our_id, receiver_id, method_name))

        if node.type == "object_creation_expression":
            for idx, child in enumerate(node.children):
                if node.field_name_for_child(idx) == "type":
                    f.instantiations.append(InstantiationFact(our_id, self._text(child)))
                    break
...
def emit_datalog(
    ...
    """
    return _Emitter(source, scope_openers, declaration_parents).emit(root)
...
def main() -> None:
    ...
    for sc in sorted(scope_ids):
        ...
        print(f"  scope [{sc:3d}] {scope_kind[sc]:<30s}  {_lookup_pos(facts, sc)}{parent_str}")
    ...
    for d in facts.declarations:
        ...
        print(f"  [{d.node_id:3d}] name={_lookup_name(facts, d.node_id)!r:<18s}  in scope [{sc}] {sc_kind}")

    print("\n=== Call graph ===")
    for c in facts.calls:
        recv = _lookup_name(facts, c.receiver_id) if c.receiver_id is not None else "<none>"
        recv_type = _lookup_type(facts, c.receiver_id) if c.receiver_id is not None else ""
        print(f"  site [{c.site_id:3d}]  {recv}.{c.method_name}()  (receiver node type: {recv_type})")
    ...
    for dt in facts.declared_types:
        print(f"  node [{dt.node_id:3d}] ({_lookup_type(facts, dt.node_id)})  : {dt.type_text}")
    ...
    for r in facts.refers_to:
        ...
        print(f"  [{r.ref_id:3d}] {ref_name!r:<12s} ({ref_pos})  →  decl [{r.decl_id}] {decl_name!r} ({decl_pos})")
