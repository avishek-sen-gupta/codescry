...
- **Embedding-based signal concretisation**: A third concretisation pipeline (`scripts/survey_repo_embedding.py`) that classifies each FILE_CONTENT integration signal by computing cosine similarity between its enclosing statement's `nomic-embed-code` embedding and 26 pre-embedded directional descriptions (13 integration types x 2 directions). Uses `extract_statement_context()` for tighter AST context (narrowest enclosing statement rather than function/class), a 0.40 similarity threshold, and argmax over all 26 descriptions to jointly determine integration type and direction. Implemented in `src/repo_surveyor/integration_concretiser/embedding_concretiser.py` with dependency-injected `EmbeddingClient` for testability. Requires `HUGGING_FACE_URL` and `HUGGING_FACE_API_TOKEN` environment variables pointing to a deployed nomic-embed-code endpoint.
