...
- **Ollama-based signal concretisation**: A parallel concretisation pipeline (`pipeline/survey_classify_by_ollama_llm.py`) that passes each unique file with detected signals to a local Ollama model, asking it to classify each signal as `DEFINITE_INWARD`, `DEFINITE_OUTWARD`, `NOT_DEFINITE`, or `NOT_INTEGRATION` with a confidence score and reason. Runs the same pre-concretisation stages as the ML pipeline; the Ollama step is implemented in `src/repo_surveyor/integration_concretiser/ollama_concretiser.py`. Supports `--model` (default: `qwen2.5-coder:7b-instruct`) and `--ollama-url` (default: `http://localhost:11434`).
- **Gemini Flash signal concretisation**: A cloud-based concretisation pipeline (`pipeline/survey_classify_by_gemini_flash_llm.py`) that mirrors the Ollama pipeline but uses Google's Gemini Flash API for classification, combining LLM reasoning with cloud-speed inference. Classifies each signal into four labels: `DEFINITE_INWARD` (exposes a service boundary), `DEFINITE_OUTWARD` (calls an external system), `NOT_DEFINITE` (real integration but direction unclear), or `NOT_INTEGRATION` (false positive). Each signal's prompt now includes its enclosing AST statement context (node type, line range, and full statement text) so the LLM can see the surrounding code structure beyond just the matched line. Uses multi-file batching to group small files into a single API call (up to 80K chars per batch), reducing API round-trips by 5-7x on typical repos. Uses the `google-genai` SDK with structured JSON response mode and exponential backoff on rate limits. Implemented in `src/repo_surveyor/integration_concretiser/gemini_concretiser.py`; shared prompt/parsing logic extracted to `llm_shared.py`. Requires `GEMINI_001_EMBEDDING_API_KEY` env var. Supports `--model` (default: `gemini-2.5-flash`).
- **Embedding-based signal concretisation**: A third concretisation pipeline (`pipeline/survey_classify_by_embedding_cosine_similarity.py`) that classifies each FILE_CONTENT integration signal by computing cosine similarity between its signal line's code embedding and 26 pre-embedded directional descriptions (13 integration types x 2 directions). Embeds the raw signal line content directly (not AST context) for best accuracy on real-world code including builder-pattern frameworks like Javalin. Uses a two-stage classification: a 0.56 similarity threshold gates signals as `SIGNAL` (real integration) or `NOISE`, then a 1.2x score ratio between the best inward and outward description scores determines `direction` (`INWARD`, `OUTWARD`, or `AMBIGUOUS` when scores are too close). AST walk-up (`batch_extract_invocation_contexts`) is still used to attach structural context to each `ConcretisedSignal` but does not influence the embedding; signals are grouped by file so each file is parsed once. Supports six embedding backends via `--backend`: **huggingface** (nomic-embed-code, requires `HUGGING_FACE_URL` and `HUGGING_FACE_API_TOKEN`), **gemini** (gemini-embedding-001, requires `GEMINI_001_EMBEDDING_API_KEY`), **ollama** (local Ollama server, default model `unclemusclez/jina-embeddings-v2-base-code`, no API key required — use `--model` and `--ollama-url` to customise), **hf-local** (local HuggingFace transformers, default model `Salesforce/codet5p-110m-embedding` producing 256-dim L2-normalised code embeddings, no API server needed — use `--model` to customise; requires optional `transformers` and `torch` dependencies, install with `poetry install -E hf-local`), **coderank** (local sentence-transformers, default model `nomic-ai/CodeRankEmbed` producing 768-dim normalised code embeddings with 8192-token context, no API server needed — use `--model` to customise; requires optional `sentence-transformers` and `torch` dependencies, install with `poetry install -E coderank`), and **bge** (local sentence-transformers, default model `BAAI/bge-base-en-v1.5` producing 768-dim normalised semantic embeddings, no API server needed — **recommended local backend**, achieves 100% classification on test repos surpassing Gemini's 91%; requires optional `sentence-transformers` and `torch` dependencies, install with `poetry install -E coderank`). The Ollama, hf-local, coderank, and bge backends run entirely locally, avoiding all API quota and rate-limit issues. BGE-base-en-v1.5 is the recommended local backend, matching or exceeding Gemini's classification quality with zero API dependency (see [EXPERIMENTS.md](EXPERIMENTS.md)). Implemented in `src/repo_surveyor/integration_concretiser/embedding_concretiser.py` with dependency-injected `EmbeddingClient`/`GeminiEmbeddingClient`/`OllamaEmbeddingClient`/`HuggingFaceLocalEmbeddingClient`/`create_coderank_embedding_client`/`create_bge_embedding_client` for testability.
- **Hybrid signal concretisation** (embedding gate + Gemini Flash direction): A hybrid pipeline (`pipeline/survey_classify_by_embedding_gate_nearest_neighbour_gemini_flash.py`) that combines the speed of embedding-based SIGNAL/NOISE gating with the accuracy of Gemini Flash for direction classification. Phase 2a runs the `GenericIntegrationDescriptionEmbeddingConcretiser` to cheaply gate signals as SIGNAL or NOISE using the cosine similarity threshold. Phase 2b sends only the SIGNALs to `concretise_with_gemini()` for direction classification (INWARD/OUTWARD/AMBIGUOUS). Phase 2c merges: embedding's validity is authoritative, Gemini's direction is authoritative for SIGNALs; if Gemini returns NOT_DEFINITE for a signal the embedding marked SIGNAL, validity stays SIGNAL but direction becomes AMBIGUOUS; if Gemini returns NOT_INTEGRATION, validity stays SIGNAL but direction becomes NOT_INTEGRATION, separating false positives from genuinely ambiguous-direction signals. Requires `GEMINI_001_EMBEDDING_API_KEY` for both Gemini embedding and Flash LLM (or `HUGGING_FACE_URL`/`HUGGING_FACE_API_TOKEN` for HuggingFace embedding backend). Supports `--backend` (huggingface/gemini) and `--output-dir` (default: `data/survey_output_hybrid`).
- **Hybrid signal concretisation** (framework-specific pattern embedding gate + Gemini Flash direction): A variant hybrid pipeline (`pipeline/survey_classify_by_framework_specific_pattern_embedding_gate_gemini_flash.py`) that uses the `FrameworkSpecificIntegrationDescriptionEmbeddingConcretiser` (~3,430 per-pattern descriptions with distance-weighted KNN) for SIGNAL/NOISE gating instead of the 26 generic directional descriptions. This gives richer semantic gating that captures framework-specific API semantics while retaining Gemini Flash's superior direction accuracy. Phase 2a runs the pattern embedding concretiser with cache and KNN; Phase 2b sends only SIGNALs to Gemini Flash for direction; Phase 2c merges with the same authority rules as the generic hybrid. Output metadata includes both pattern-embedding fields (`nearest_description`, `nearest_type`, `nearest_direction`, `nearest_source`, `nearest_score`, `nearest_k`, `nearest_k_avg_score`) and Gemini fields (`gemini_confidence`, `gemini_reason`). Supports all six embedding backends via `--backend`, `--threshold` (default: 0.68), `--k` (default: 5), and auto-timestamped output directories. Requires `GEMINI_001_EMBEDDING_API_KEY`.
- **Pattern-embedding signal concretisation**: A fourth concretisation pipeline (`pipeline/survey_classify_by_embedding_similarity_weighted_knn.py`) that classifies each FILE_CONTENT integration signal by nearest-neighbor lookup against pre-embedded per-pattern descriptions. Unlike the generic embedding concretiser which uses 26 directional descriptions (one per integration type x direction), this pipeline pre-embeds all ~3,430 pattern description strings (two per pattern across ~1,371 patterns) via `get_all_pattern_descriptions()`, yielding much richer semantic targets that capture framework-specific API semantics. Each signal is embedded using its **statement-level AST context** (`batch_extract_statement_contexts`) rather than the raw matched line or innermost invocation node — signals are grouped by file so each file is parsed once — this captures the full enclosing statement (e.g. an entire `app.get(...)` Javalin route registration rather than just `ctx.path()` inside a lambda), giving the embedding model enough context for accurate nearest-neighbor matching. Statement detection uses suffix heuristics (`_statement`, `_definition`) supplemented by **per-language node type mappings** that handle expression-oriented languages (Rust, Kotlin, Scala use `_expression` suffix for control flow), suffix-free languages (Ruby uses bare `if`, `while`, `case`), and COBOL (`_header` suffix). Rust's tree-sitter grammar also uses `*_item` suffix for top-level definitions (`function_item`, `struct_item`, `impl_item`, etc.) instead of the `*_definition`/`*_declaration` convention used by most other languages; these are registered as definition nodes so the walk-up correctly resolves Warp-style method chains to their enclosing function. Classification uses **distance-weighted K-nearest-neighbours (K=5 default, configurable via `--k`)**: validity is gated by the average cosine score of the K neighbours (more robust than a single-nearest threshold), and direction is determined by a distance-weighted vote among the K neighbours (closer matches have more influence; ties break to the highest-scoring individual neighbour). Before threshold comparison, an **evidence check system** (`evidence.py`) adjusts the raw cosine score using contextual heuristics: seven universal suppression checks (`in_test_file` -0.15, `in_vendor_dir` -0.25, `in_generated_file` -0.30, `in_config_dir` -0.10, `in_string_literal` -0.30, `in_log_statement` -0.25, `in_constant_decl` -0.20) penalise signals in noisy contexts, while the framework accepts an extensible `framework_evidence` parameter for pattern-source-specific boost/suppress checks. Evidence weights accumulate and the adjusted score is clamped to [0, 1]. Signals below the cosine similarity threshold (default 0.62) after evidence adjustment are classified as NOISE/AMBIGUOUS. Metadata per signal includes `nearest_description`, `nearest_type`, `nearest_direction`, `nearest_source`, `score` (top-1 aliases for backward compatibility), plus KNN fields: `k`, `avg_score`, `adjusted_avg_score`, `neighbours` (list of K neighbours with description/type/direction/source/score), `direction_votes` (per-direction weighted sums), `score_adjustment`, and `evidence_fired`. **Vectorized KNN**: Cosine similarity computation is vectorized via numpy matrix multiplication (`_find_all_k_nearest_batch`), computing the full (N_signals x M_descriptions) similarity matrix in a single pass rather than per-signal pure-Python loops — reducing classification time from ~215s to ~0.3s (741x speedup) on 758 signals against 3,478 descriptions. **Embedding cache**: Pattern description embeddings are cached to `data/embeddings/pattern_description_embeddings_{backend}.json` and auto-loaded on subsequent runs, skipping the ~2 minute embedding API warm-up. The cache is keyed by a SHA-256 content hash of all description texts, so it auto-invalidates when patterns change. Pre-build the cache with `poetry run python pipeline/survey_build_pattern_embedding_cache.py --backend gemini`. Implemented in `src/repo_surveyor/integration_concretiser/pattern_embedding_concretiser.py`. Supports `--backend` (huggingface/gemini/ollama/hf-local/coderank/bge), `--threshold`, and `--k` (number of nearest neighbours, default 5).
- **Pipeline timing instrumentation**: All 7 pipeline scripts write a `timings.json` file to the output directory capturing per-stage durations. Phase 1 stages (pre-concretisation via `survey()`) are timed internally by `PipelineTimingObserver`; Phase 2 stages (embedding gate, LLM concretisation, merge) are now also instrumented, so the most expensive operations (e.g. embedding inference ~33s, Gemini API calls ~250s) are visible in the timing output. Stage names use undotted conventions (e.g. `embedding_gate`, `gemini_direction`, `pattern_embedding_concretisation`) so they are included in the `total_seconds` aggregate.
